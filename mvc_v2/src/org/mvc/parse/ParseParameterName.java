...
/**
 * 用于解析出类的方法和方法对应的参数的名称
 * @author Administrator
 *
 */
public class ParseParameterName {
	
	
	/**
	 * 根据类型获取对应的 key：方法名称 value：方法的参数名称
	 * @param clazz
	 * @return
	 */
	public static Map<String, List<String>> getParameNamesByClass(Class clazz){
		InputStream in = clazz.getResourceAsStream("/" + clazz.getName().replace('.', '/') + ".class");
		Map<String, List<String>> map = null;
		try {
			map =  getParamNames(in);
		} catch (IOException e) {
			e.printStackTrace();
		}
		return map;
